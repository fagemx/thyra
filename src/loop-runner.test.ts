...
describe('LoopRunner', () => {
  ...
  it('decide: returns null in Phase 0', async () => {
    const result = await loopRunner.decide(
      { id: chiefId, village_id: villageId, name: 'test', role: 'r', version: 1, status: 'active', skills: [], pipelines: [], permissions: [], personality: { risk_tolerance: 'moderate', communication_style: 'concise', decision_speed: 'fast' }, constraints: [], profile: null, created_at: '', updated_at: '' },
      [],
      [],
    );
  ...
  describe('decide with EddaBridge', () => {
    it('queries Edda for precedents when bridge is provided', async () => {
      ...
      const chief = { id: chiefId, village_id: villageId, name: 'test', role: 'r', version: 1, status: 'active' as const, skills: [], pipelines: [], permissions: [], personality: { risk_tolerance: 'moderate' as const, communication_style: 'concise' as const, decision_speed: 'fast' as const }, constraints: [], profile: null, created_at: '', updated_at: '' };
