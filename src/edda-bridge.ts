...
export class EddaBridge {
  ...
  async recordDecision(decision: EddaDecision): Promise<boolean> {
    try {
      const res = await fetch(`${this.eddaUrl}/api/decisions`, {
        method: 'POST',
      ...
      if (res.ok) {
        appendAudit(this.db, 'edda', `${decision.domain}.${decision.aspect}`, 'record', decision, 'system');
        return true;
      }
      return false;
    } catch {
      // Graceful degradation: log warning, don't crash
      appendAudit(this.db, 'edda', `${decision.domain}.${decision.aspect}`, 'record_failed', { ...decision, error: 'Edda unreachable' }, 'system');
      return false;
    }
  }
