...
describe('DecisionEngine golden fixtures (Blog Village)', () => {
  ...
  describe('cold start（無 intent、無歷史）', () => {
    it('應根據 active laws 啟動 pipeline，dispatch research', async () => {
    ...
    it('沒有 active laws 時，action 為 null', async () => {
      const ctx = makeColdStartContext(
  ...
  describe('mid-pipeline（research → draft）', () => {
    ...
    it('updated_intent 應反映新階段', async () => {
      const ctx = makeMidPipelineContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const result = await engine.decide(ctx);
    ...
    it('estimated_cost 不超過 per_action_limit', async () => {
      const ctx = makeMidPipelineContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const result = await engine.decide(ctx);
  ...
  describe('budget exhausted（budget_ratio < 0.1）', () => {
    ...
    it('estimated_cost 為 0（不再花費）', async () => {
      const ctx = makeBudgetExhaustedContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const result = await engine.decide(ctx);
  ...
  describe('negative precedent（Edda harmful verdict）', () => {
    it('confidence 應低於基線', async () => {
      // 無先例的基線 context
      const baseCtx = makeMidPipelineContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const baseResult = await engine.decide(baseCtx);

      // 含負面先例
      const negCtx = makeNegativePrecedentContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const negResult = await engine.decide(negCtx);
    ...
    it('precedent_notes 應包含負面先例', async () => {
  ...
  describe('law rollback（recent_rollbacks >= 3）', () => {
    it('應產生 law proposal（策略調整）', async () => {
    ...
    it('law proposal 的 evidence 應有 reasoning', async () => {
      const ctx = makeLawRollbackContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const result = await engine.decide(ctx);
  ...
  describe('personality effect（conservative chief）', () => {
    it('personality_effect 應包含 conservative', async () => {
      const ctx = makeColdStartContext(
        seed.chief, seed.constitution, seed.skills, seed.laws,
      );
      const result = await engine.decide(ctx);
    ...
    it('低預算 conservative chief 應傾向 complete_cycle', async () => {
